...
public class SelectSawonTable extends JFrame implements ActionListener,ItemListener{
	...
	{
		String [] title= {"등록번호","이름","사원번호","부서","직급","급여","주소"};
		...
		//라디오버튼추가
				...
				String [] rb_label= {"전체","개발부","기획부","홍보부","부장","과장","대리"};
	...
	public void sawonTableSelect(int select)
	{
		...
		if(select==1)
			sql="select num,name,empno,buseo,position,to_char(pay,'L999,999,999') pay,addr from sawon order by num";
		
		else if(select==2)
			sql="select num,name,empno,buseo,position,to_char(pay,'L999,999,999') pay,addr from sawon where buseo='개발부' order by num";
				
		else if(select==3)
			sql="select num,name,empno,buseo,position,to_char(pay,'L999,999,999') pay,addr from sawon where buseo='기획부' order by num";
		
		else if(select==4)
			sql="select num,name,empno,buseo,position,to_char(pay,'L999,999,999') pay,addr from sawon where buseo='홍보부' order by num";
		
		else if(select==5)
			sql="select num,name,empno,buseo,position,to_char(pay,'L999,999,999') pay,addr from sawon where position='부장' order by num";
		
		else if(select==6)
			sql="select num,name,empno,buseo,position,to_char(pay,'L999,999,999') pay,addr from sawon where position='과장' order by num";
		
		else if(select==7)
			sql="select num,name,empno,buseo,position,to_char(pay,'L999,999,999') pay,addr from sawon where position='대리' order by num";
	...
	public void insertScoreData()
	{
		...
		String empno=addSawon.tfEmpno.getText();
	...
	public void isOneStuData(String num)
	{
		...
		try {
			...
			if(rs.next())
			{
				updateSawon.num=num; //hidden
				updateSawon.tfName.setText(rs.getString("name"));
				updateSawon.tfEmpno.setText(rs.getString("empno"));
	...
	public void update()
	{
		...
		String empno=updateSawon.tfEmpno.getText();
		...
		String sql="update sawon set name=?,empno=?,buseo=?,position=?,pay=?,addr=? where num=?";
	...
	public void actionPerformed(ActionEvent e) {
		...
		{	
			...
			boolean a = checkIfSawonExist(num);
		        ...
		        if (a==true) 
		        {
		            this.isOneStuData(num);  // 등록번호에 해당하는 사원 정보를 수정 폼에 띄움
