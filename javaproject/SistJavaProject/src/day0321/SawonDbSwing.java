...
public class SawonDbSwing extends JFrame implements ItemListener,ActionListener{
	...
	public SawonDbSwing(String title) {
		...
		//테이블 생성 후 db로부터 데이터 가져오기(1:전체출력)
	...
	public void initDesign()
	{
		...
		String [] rb_label= {"전체","남자","여자","홍보부","개발부","기획부"};
		...
		btnDel.addActionListener(this); //이벤트 추가
		this.add(btnDel);
	...
	public void sawonTableSelect(int select)
	{
		...
		if(select==1)
			...
			sql="select ROWNUM no,num id,name,gender,buseo,to_char(pay,'L999,999,999') pay from sawon where buseo='기획부'";
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(ob==btnAdd)
		{
			...
			String buseo=JOptionPane.showInputDialog("부서를 입력해주세요(홍보부,개발부,기획부)");
			String pay=JOptionPane.showInputDialog("월급여를 입력해주세요");
		...
		{
			String num=JOptionPane.showInputDialog("삭제할 사원ID를 입력하세요");
